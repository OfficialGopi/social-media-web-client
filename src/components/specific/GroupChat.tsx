    ...
    <>
      {Array.from(Array(30)).map((val: any, index: number) => (
        <GroupChatSpecific
          val={val}
          key={index}
          to={(index * 100 + 100).toString()}
        />
      ))}
    </>
  );
};
const GroupChatSpecific = ({ val, to }: { val: any; to: string }) => {
  return (
    <NavLink
      className={({ isActive }) =>
        isActive
    ...
    >
      ...
      <div className="flex flex-col   h-full w-[calc(100%-56px)] items-start">
        <span className="text-base text-white">Group</span>
        <span className="text-sm text-gray-500">You: This is Weird..</span>
      </div>
    </NavLink>
  );
};
